"""Surface Poisson operator: FEM Laplace-Beltrami solve on a closed sphere surface.

Provides ``solve_surface`` which assembles and solves

    eps * a(u, v) = L(v)   for all v in V

on a closed 2-sphere surface mesh embedded in R^3, where

    a(u, v) = integral( inner(grad(u), grad(v)) ) dx
    L(v)    = integral( f * v ) dx    (consistent)
            = sum_i mL_i * f_i * phi_i   (lumped mass)
...
Closed-surface gauge fix:
  The pure-Neumann system has a constant null space; we pin one DOF (index 0)
  to the exact value there (or 0 if exact_fn is None) for solvability.  Error
  norms use area-weighted mean subtraction to remove the gauge ambiguity.

dolfinx 0.10.0 API notes (confirmed in Task 1 + Task 3):
